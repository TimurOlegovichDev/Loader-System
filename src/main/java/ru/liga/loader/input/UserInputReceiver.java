package ru.liga.loader.input;

import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Service;

import java.util.Scanner;

@Slf4j
@Service
public class UserInputReceiver {

    private final Scanner scanner = new Scanner(System.in);

    /**
     * Получает целое число от пользователя.
     * Этот метод выводит сообщение на консоль, ожидает ввода от пользователя и возвращает введенное целое число.
     * Если пользователь вводит не целое число, то метод выводит сообщение об ошибке и запрашивает ввод снова.
     *
     * @param msg сообщение, выводимое на консоль
     * @return введенное целое число
     */

    public int getNumber(String msg) {
        log.info(msg);
        while (true) {
            try {
                int number = Integer.parseInt(scanner.nextLine());
                log.debug("Введено число: {}", number);
                return number;
            } catch (NumberFormatException e) {
                log.error("Число введено неверно, попробуйте еще раз");
            }
        }
    }

    /**
     * Получает строку от пользователя.
     * Этот метод выводит сообщение на консоль, ожидает ввода от пользователя и возвращает введенную строку.
     *
     * @param msg сообщение, выводимое на консоль
     * @return введенная строка
     */

    public String getInputLine(String msg) {
        log.debug("Запрос на ввод со следующим сообщением: {}", msg);
        log.info(msg);
        String input = scanner.nextLine();
        log.debug("Пользовательский ввод: {}", input);
        return input;
    }
}
